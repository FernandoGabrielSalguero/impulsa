...
class UserAvatarStorageService
{
    ...
    public function store(UserAuth $user, UploadedFile $file): string
    {
        if (! $file->isValid()) {
        ...
        $previousPath = trim((string) $user->info?->avatar_path);
    ...
    public function resolveMimeType(string $absolutePath): string
    {
    ...
    {
        return (string) config('uploads.user_avatar.storage_path');
    }

    private function pathPrefix(): string
    {
        return (string) config('uploads.user_avatar.path_prefix', 'user-avatars');
    }
}
